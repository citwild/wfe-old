...
func Init(
  ...
  logger = Logger{trace: Trace, info: Info, warning: Warning, error: Error}
}
...
func userAuth(c *gin.Context) {
  var form Login
  logger.info.Println("Authorizing user")
  if c.Bind(&form) == nil {
    ...
    if err != nil {
      ...
      if _, ok := resp.Item["Email"]; ok {
        if *resp.Item["Email"].S == form.Email && *resp.Item["Password"].S == form.Password {
          ...
          logger.info.Println("User password and email match")
          c.HTML(http.StatusOK, "bucketlist.tmpl", gin.H{
            "bucketlist": bucketlist,
          })
        } else {
          logger.info.Println("Failure authorizing user: Invalid login")
          c.HTML(http.StatusUnauthorized, "index.tmpl", gin.H{
    ...
    logger.info.Println("Failure authorizing user: No input provided")
    c.HTML(http.StatusUnauthorized, "index.tmpl", gin.H{
...
func bucketShow(c *gin.Context) {
  ...
  if c.Bind(&bucket) == nil {
    ...
    if err != nil {
      logger.error.Println("Failed to list objects", err)
    }
  } else {
    logger.info.Println("No buckets selected")
  }
}
